...
const ProfileSelector: React.FC<Props> = ({ onProfileSelected }) => {
  const { profiles, currentProfile, selectProfile, addProfile, deleteProfile, clearCurrentProfile, forceCreateDefaultProfiles } = useProfiles();
  const { t } = useLanguage();
  
  console.log('🎭 ProfileSelector - profiles recibidos:', profiles);
  console.log('🎭 ProfileSelector - currentProfile:', currentProfile);
  console.log('🎭 ProfileSelector - profiles.length:', profiles.length);
  ...
  // Mostrar loading si no hay perfiles cargados
  if (profiles.length === 0) {
    return (
      <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50">
        <div className="text-center">
          <h1 className="text-6xl font-bold text-white mb-12">SirNet</h1>
          <div className="text-white text-xl mb-4">Cargando perfiles...</div>
          <button 
            onClick={forceCreateDefaultProfiles}
      ...
      <div className="text-center max-w-4xl mx-auto px-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 mb-8">
          {profiles.map(profile => (
            <div key={profile.id} className="group cursor-pointer transition-transform hover:scale-105" onClick={() => handleSelectProfile(profile)}>
          ...
          {profiles.length < 5 && (
            <div className="cursor-pointer transition-transform hover:scale-105" onClick={() => setIsAddingProfile(true)}>
            ...
            <div className="bg-gray-900 rounded-lg p-6 max-w-md w-full mx-4">
              ...
              <div className="mb-4">
                ...
                <div className="grid grid-cols-7 gap-2">
                  {avatars.map(avatar => (
                    <button key={avatar} onClick={() => setSelectedAvatar(avatar)} className={`w-10 h-10 rounded-lg flex items-center justify-center text-xl transition-colors ${selectedAvatar === avatar ? 'bg-blue-600' : 'bg-gray-700 hover:bg-gray-600'}`}>
                      {avatar}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex gap-3">
                <button onClick={() => setIsAddingProfile(false)} className="flex-1 px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors">
                  {t('cancel')}
                </button>
                <button onClick={handleAddProfile} disabled={!newProfileName.trim()} className="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white rounded-lg transition-colors">
                  {t('save-profile')}
                </button>
              </div>
            </div>
